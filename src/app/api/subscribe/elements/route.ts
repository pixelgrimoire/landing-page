...
export async function POST(req: NextRequest) {
  try {
    ...
    if (paymentIntentCS) {
      return new Response(
        JSON.stringify({ client_secret: paymentIntentCS, subscription_id: subscription.id, intent_type: 'payment' }),
    ...
    if (setupIntentCS) {
      return new Response(
        JSON.stringify({ client_secret: setupIntentCS, subscription_id: subscription.id, intent_type: 'setup' }),
        { status: 200, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    return new Response(
      JSON.stringify({ client_secret: si.client_secret, subscription_id: subscription.id, intent_type: 'setup', fallback: true }),
      { status: 200, headers: { 'Content-Type': 'application/json' } }
    );
  } catch (e: unknown) {
